...
var Transport = function(wsdl, options, callback) {
  ...
  this.getParams = function(function_name, value) {
    var response = "";
    switch (function_name) {
      case 'DaneKomunikat':
      ...
      case 'DanePobierzPelnyRaport':
        response = '<tns:DanePobierzPelnyRaport><tns:pRegon>' + value + '</tns:pRegon><tns:pNazwaRaportu>PublDaneRaportPrawna</tns:pNazwaRaportu></tns:DanePobierzPelnyRaport>';
        break;
      case 'GetValue':
  ...
  this.getParamValue = function(param_name, value) {
    ...
    switch (_.lowerCase(param_name).replace(/\s/g, '')) {
      ...
      case 'nipy':
        ...
        break;
      case 'regon':
        response = '<dat:Regon>' + value + '</dat:Regon>';
        break;
      case 'regony14zn':
        response = '<dat:Regony14zn>' + value + '</dat:Regony14zn>';
        break;
      case 'regony9zn':
        response = '<dat:Regony9zn>' + value + '</dat:Regony9zn>';
        break;
      default:
        response = '<dat:Regony9zn>' + value + '</dat:Regony9zn>';
        break;
